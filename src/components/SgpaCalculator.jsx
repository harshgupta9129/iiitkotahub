...
import { Calculator, BookOpen, GraduationCap, ChevronRight, CheckCircle2, RotateCcw } from "lucide-react"; // Add: npm install lucide-react
...
const SgpaCalculator = () => {
  ...
  const allGradesSelected = subjects.length > 0 && subjects.every((sub) => subjectState[sub.code]?.grade !== "");
      ...
      <div className="max-w-6xl mx-auto px-4 py-12">
        ...
        >
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-purple-500/10 border border-purple-500/20 text-purple-400 text-xs font-medium mb-4">
            <GraduationCap size={14} /> IIIT Kota Portal
          </div>
          ...
          <p className="mt-4 text-gray-400 max-w-xl mx-auto text-sm md:text-base leading-relaxed">
            Fast, accurate, and optimized for both Information Technology and Computer Science branches.
          </p>
        </motion.div>
          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-2xl">
              ...
              <div className="space-y-5">
                <div className="group">
                  <label className="block text-xs font-medium text-gray-400 mb-2 group-focus-within:text-purple-400 transition-colors">Select Semester</label>
                  <select
                    value={semester}
                    onChange={(e) => { setSemester(e.target.value); setBranch(""); setCurrentStep(2); }}
                ...
                <div className="group">
                  <label className="block text-xs font-medium text-gray-400 mb-2 group-focus-within:text-purple-400 transition-colors">Choose Branch</label>
                  <select
                    value={branch}
                    disabled={!semester}
                    onChange={(e) => { setBranch(e.target.value); setCurrentStep(3); }}
                ...
                {branch && (
                  <button onClick={reset} className="w-full flex items-center justify-center gap-2 text-xs text-gray-500 hover:text-white transition-colors pt-2">
            ...
            <AnimatePresence>
              {sgpa && (
                ...
                >
                  <p className="text-purple-200 text-sm font-medium mb-1 uppercase tracking-tighter">Current SGPA</p>
            ...
            <AnimatePresence mode="wait">
              {currentStep >= 3 ? (
                ...
                >
                  <div className="flex items-center justify-between mb-2 px-2">
                    <h2 className="text-xl font-bold flex items-center gap-2">
                      <CheckCircle2 className="text-purple-500" size={20} /> Subject Grades
                    </h2>
                    <span className="text-xs text-gray-500">{subjects.length} Subjects Detected</span>
                  </div>
                      ...
                      >
                        ...
                        <select
                          ...
                          className={`w-full bg-[#030211] border rounded-xl px-4 py-3 text-sm focus:outline-none transition-all appearance-none
                            ${subjectState[sub.code]?.grade ? 'border-purple-500/50 text-white' : 'border-white/10 text-gray-500'}`}
                        >
                  ...
                  {sgpa && (
                    <div className="lg:hidden mt-6 bg-gradient-to-br from-purple-600 to-indigo-700 rounded-2xl p-8 text-center shadow-2xl">
                      <p className="text-purple-200 text-sm font-medium mb-1">Your Result</p>
                ...
                <div className="h-full flex flex-col items-center justify-center text-center p-12 border-2 border-dashed border-white/5 rounded-3xl">
                  ...
                  <h3 className="text-lg font-medium text-gray-500">Please select a semester & branch to begin</h3>
                  <p className="text-sm text-gray-600 mt-2">Your subject list will appear here automatically.</p>
                </div>
              )}
            </AnimatePresence>
          </div>
